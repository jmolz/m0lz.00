---
title: "m0lz.03 Research"
description: "Benchmark methodology, threat modeling, and security analysis for the m0lz.03 security proxy daemon."
project: "m0lz.03"
author: "Jacob Molz"
date: "2026-04-10"
sections:
...
- **High detection on in-scope attacks** — the patterns being matched are explicit and well-defined. When m0lz.03 is looking for a denied tool name or a regex-matched SSN, it finds it.
...
**This benchmark tests m0lz.03 against its own scenario generators.** We are aware that this is the most common source of inflated security claims. Rather than downplaying this, we want to explain how we mitigate it and why we believe the results are still informative.
...
| MCPSecBench Category | m0lz.03 Coverage | Notes |
|---------------------|--------------------|-------|
...
We acknowledge that production traffic will include patterns not in this set. If you encounter a false positive, please [open an issue](https://github.com/jmolz/m0lz.03/issues).
...
We investigated cross-validating m0lz.03 against GenTelLab's MCP-AttackBench (the largest published MCP security dataset at 70K+ samples). The dataset is incompatible with protocol-layer evaluation:
...
- **Threat model mismatch** — MCP-AttackBench evaluates whether a model can be tricked into making malicious calls. m0lz.03 evaluates whether a malicious call (regardless of origin) gets blocked at the protocol layer. These are complementary defenses at different layers, not competing approaches to the same problem.
...
m0lz.03's benchmark suite remains self-generated. We mitigate this with: open-source reproducibility (`pnpm benchmark`), programmatic generation with documented methodology, per-category transparency (showing 92.4%–100% range rather than a single aggregate), and explicit limitations disclosure.
...
**Complementary MCP security benchmarks.** These two papers evaluate agent-layer resilience against malicious MCP server instructions — a distinct but neighboring threat model to m0lz.03's transport-layer proxy scope.

- **MCPSecBench** — *A Comprehensive Benchmark for Evaluating MCP Security.* 17 attack types across 4 attack surfaces. [arxiv.org/abs/2508.13220](https://arxiv.org/abs/2508.13220)
- **MSB (MCP Security Bench)** — *Benchmarking LLM Agent Robustness Against Malicious MCP Servers.* 12 attack types, 2,000 instances. ICLR 2026. [arxiv.org/abs/2510.15994](https://arxiv.org/abs/2510.15994)
